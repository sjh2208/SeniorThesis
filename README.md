...
Where the x’s represent the input data of each dot and the w’s are the ‘weights’, all of these products summing to the output y. This output y then undergoes an ‘activation function,’ which transforms the continuous output to a discrete one—either 0 or 1. Keeping with our above example, the problem now is: given a ‘blue dot’ represented by a vector of x’s, what are the appropriate weights that will transform that input data in such a way that the output becomes 0? In a different light, this process is a matrix multiplication:
$$X^T \times W = \begin{bmatrix}1 & x_1 & x_2  & \cdots  & x_n \end{bmatrix} \times \begin{bmatrix} w_0 
w_1 
w_2 
\vdots 
w_n \end{bmatrix} =[y]$$
...
Dechêne, A., Stahl, C., Hansen, J., & Wänke, M. (2010). The truth about the truth: a meta-analytic review of the truth effect. Personality and social psychology review, 14(2), pp. 238–257.
...
Hasher, L., Goldstein, D., & Toppino, T. (1977). Frequency and the conference of referential validity. Journal of Verbal Learning & Verbal Behavior, 16(1), pp. 107–112.
